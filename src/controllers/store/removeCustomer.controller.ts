...
export function removeCustomer (req: Request, res: Response){
    ...
    const customerTime = data.customerTime;
        ...
        connection.query(`UPDATE queues SET estimatedWaitTime = estimatedWaitTime - ${customerTime} WHERE store = ${store_id} AND position > ${userPosition};UPDATE queues SET position = position - 1 WHERE store = ${store_id} AND position > ${userPosition};DELETE FROM queues WHERE userID = ${user_ID};`, async (err:any, result:any) => {
